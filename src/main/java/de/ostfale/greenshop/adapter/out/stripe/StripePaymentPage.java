 ...
 * so the amount is Stripe's and not the browser's — then a checkout session for one piece of
 * it. Stripe answers with the address of its hosted page.
 * <p>
 * The success address carries {@code {CHECKOUT_SESSION_ID}}, which Stripe replaces with the
 * id of the session on the way back; the web adapter reads it as {@code session_id}.
 */
@Component
class StripePaymentPage implements PaymentPage {

    static final String SUCCESS_PATH = "/checkout/success?session_id={CHECKOUT_SESSION_ID}";
    static final String CANCEL_PATH = "/checkout/cancel";
